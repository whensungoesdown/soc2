...
static Fat* find_fat_volume(const char* name, int len)
{
  ...
  {
    if (len == it->name_len && !u_memcmp(name, it->name, len))
      return it;
  }
...
static int dir_search(Dir* dir, const char* name, int len, Loc* loc)
{
  ...
  for (int err = 0;; err = dir_next(dir))
  {
    ...
    if (sfn_is_lfn(sfn))
    {
      ...
      if (g_len == len && !u_memcmp(g_buf, name, len))
        return FAT_ERR_NONE;
    }
    else
    {
      if (!u_memcmp(sfn_name, sfn->name, sizeof(sfn_name)))
        return FAT_ERR_NONE;
    }
  }
}
...
static int dir_add(Dir* dir, const char* name, int len, uint8_t attr, uint32_t clust)
{
  ...
  u_memcpy(sfn->name, sfn_name, sizeof(sfn->name));
...
//    return false;
  ...
  if (u_memcmp(bpb->fs_type, "FAT32   ", 8))
    return false;
...
static inline int u_ctz(unsigned int x) {
    if (x == 0) return 32;
    int n = 0;
    while ((x & 1) == 0) {
        n++;
...
int fat_mount(DiskOps* ops, int partition, Fat* fat, const char* name)
{
  ...
  fat->clust_shift = u_ctz(bpb->sect_per_clust);
  ...
  int name_len = u_strlen(name);
  if (name_len > sizeof(fat->name))
    return FAT_ERR_PARAM;
  u_memcpy(fat->name, name, name_len);
...
int fat_dir_create(Dir* dir, const char* path)
{
  ...
  u_memset(sfn[0].name, ' ', sizeof(sfn->name));
  sfn[0].name[0] = '.';
...
int fat_dir_read(Dir* dir, DirInfo* info)
{
  ...
  for (int err = 0;; err = dir_next(dir)) // Hack to allow continue
  {
    ...
    u_memcpy(info->name, g_buf, g_len);
    info->name_len = g_len;
